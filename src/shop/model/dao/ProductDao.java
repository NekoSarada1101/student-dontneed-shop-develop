...
public class ProductDao extends DaoBase {
    ...
    public List<ProductBeans> fetchSearchProductList(int genreCode, String sortColumn, String sortOrder, String searchWord) {
        ...
        try {
            ...
            if (genreCode == 0) { //ジャンルですべてを指定された場合
                stmt = con.prepareStatement("SELECT * FROM product WHERE is_sold = false AND product_name LIKE ? ORDER BY " + sortColumn + " " + sortOrder);
                stmt.setString(1, searchWord);
            } else {
                stmt = con.prepareStatement("SELECT * FROM product WHERE genre_code = ? AND is_sold = false AND product_name LIKE ? ORDER BY " + sortColumn + " " + sortOrder);
    ...
    public List<ProductBeans> fetchAdminSearchProductList(String adminMail, int genreCode, String sortColumn, String sortOrder, String searchWord) {
        ...
        try {
            ...
            if (genreCode == 0) { //ジャンルですべてを指定された場合
                stmt = con.prepareStatement("SELECT * FROM product WHERE admin_mail = ? AND product_name LIKE ? ORDER BY " + sortColumn + " " + sortOrder);
                stmt.setString(1, adminMail);
                stmt.setString(2, searchWord);
            } else {
                stmt = con.prepareStatement("SELECT * FROM product WHERE admin_mail = ? AND genre_code = ? AND product_name LIKE ? ORDER BY " + sortColumn + " " + sortOrder);
                stmt.setString(1, adminMail);
